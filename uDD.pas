...
const
  ...
  defAppTokenProducao        = '51ac38ed-b08f-11e4-a156-064a56311ffd'; // app token produÁ„o
...
uses
  ufrmAviso,
  uMain,
  ufrmAguardaServidor, ufrmRequest, frmconfiguracoes, UFrmSenhaManutencao;
...
begin
  ...
  begin
    ...
    begin
      ...
      frmconfig.ShowModal;  // <== espera atÈ o usu·rio fechar

      FreeAndNil(frmconfig);  // opcional: libera da memÛria apÛs fechar
    end;
  end;
  ...
  defServer             := DD.config.ReadString('Config', 'server', '');

  defTimerAtualizacao   := QrConfigTEMPO_ATUALIZA.Value * 1000;
  defServer  := QrConfigSERVER.Value;
end;
...
function TiraAcento(A: string): string;
const
  ComAcento = '¡…Õ”⁄·ÈÌÛ˙¿»Ã“Ÿ‡ËÏÚ˘›˝√’„ı¬ Œ‘€‚ÍÓÙ˚ƒÀœ÷‹‰ÎÔˆ¸«Áˇ—Ò';
