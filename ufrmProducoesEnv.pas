...
type
  TfrmProducoesEnv = class(TForm)
    ...
    procedure DtProducoesDataChange(Sender: TObject; Field: TField);
  private
...
procedure TfrmProducoesEnv.btnFiltrarClick(Sender: TObject);
var
  DataInicial, DataFinal: TDateTime;
  CampoFiltrar: string;
begin
  DataInicial := Trunc(dtInicio.Date);
  DataFinal := Trunc(dtFim.Date) + 1 - (1 / (24 * 60 * 60)); // até 23:59:59

  // Determina o campo a ser filtrado com base na seleção do usuário
  case cbCampo.ItemIndex of
    0: CampoFiltrar := 'earlyStart';
    1: CampoFiltrar := 'laterEnd';
    2: CampoFiltrar := 'deliveryDate';
  else
...
begin
    if Assigned(frmMain.btnAtualizacaoAutomaticaTudo.OnClick) then
...
procedure TfrmProducoesEnv.btnTodosClick(Sender: TObject);
begin
  with QrProducoes do
  begin
    Close;
...
procedure TfrmProducoesEnv.DtProducoesDataChange(Sender: TObject; Field: TField);
begin

end;

//Limpa o FORM ao fechar
procedure TfrmProducoesEnv.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  frmProducoesEnv := nil;
end;
...
procedure TfrmProducoesEnv.FormCreate(Sender: TObject);
begin
